...
final class AppDelegateTests: XCTestCase {
    ...
    private final class WindowHarness {
        ...
        private func makeIsolatedDefaults() -> UserDefaults {
            let suiteName = "FoveaAppTests.WindowHarness.\(UUID().uuidString)"
    ...
    func testMainMenuLocalizesSubmenuItemsForSimplifiedChinese() {
        ...
        XCTAssertNotNil(menu.items[5].submenu?.item(withTitle: "Fovea 帮助"))
    }
    ...
    private func makeIsolatedDefaults() -> UserDefaults {
        let suiteName = "FoveaAppTests.AppDelegate.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)
        return defaults
    }
