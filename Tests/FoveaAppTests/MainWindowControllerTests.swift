...
final class MainWindowControllerTests: XCTestCase {
    ...
    func testInspectorMenuValidationReflectsSettingState() {
        let defaults = UserDefaults(suiteName: "FoveaAppTests.Inspector.\(UUID().uuidString)")!
        let settings = AppSettings(defaults: defaults)
        let controller = MainWindowController(window: nil, settings: settings)
    ...
    func testTitleBarGridButtonShowsTheFolderNameInTheTitle() async throws {
        ...
        let controller = MainWindowController(settings: AppSettings(defaults: makeIsolatedDefaults()))
        controller.open(url: imageURL)
        for _ in 0..<100 where controller.window?.title == "Fovea" {
            try await Task.sleep(for: .milliseconds(10))
        }
    ...
    private func makeIsolatedDefaults() -> UserDefaults {
        UserDefaults(suiteName: "FoveaAppTests.MainWindowController.\(UUID().uuidString)")!
    }
}
